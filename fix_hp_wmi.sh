...
if command -v dkms &>/dev/null; then
    ...
    DKMS_VER=$(grep -oP 'PACKAGE_VERSION="\K[^"]+' driver/dkms.conf 2>/dev/null || echo "1.1.4")
...
if [ "$FOUND_BACKUP" = false ]; then
    echo -e "\033[0;31m[!] No .backup driver found. Your stock driver might already be active or missing from /lib/modules/$KVER/.\033[0m"
else
